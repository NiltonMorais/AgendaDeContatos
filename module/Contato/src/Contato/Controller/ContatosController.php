...
public function searchAction()  // action para fazer o autocomplete na BUSCA
{
    $nome = $this->params()->fromQuery('query', null);
    if (isset($nome)) {
        $result = $this->getContatoTable()->search($nome);   // dependencia do módulo search em ContatoTable.php
    } else  {
        $result = [];  
    }
    ...
    {
        return ['formContato' => new ContatoForm()]; // formulário criado em Form/ContatoForm.php
    }
    ...
    public function detalhesAction()
{
    ...
    try {
      ...
      // antes do cache =>   $contato = $this->getContatoTable()->find($id);
    ...
    // dados eviados para detalhes.phtml  ( se for por modal vai sem o layout )
    return (new ViewModel())
    ...
    if ($request->isPost()) {
        ...
        if ($form->isValid()) {
            ...
            // verifica se existe cache para o item que foi atualizado, se sim ele é deletado pois é o cache antigo, e será gerado um novo cache no action detalhes
    ...
    public function deletarAction()
    {
        // filtra id passsado pela url
         ...
         if (!$id) {
            ...
            // verifica se existe cache para o item, se sim ele é deletado
            $nome_cache_contato_id = "nome_cache_contato_$id";
            if ($this->cache()->hasItem($nome_cache_contato_id)) 
                {
                     $this->cache()->removeItem($nome_cache_contato_id);
